...
apt update && apt install -y ca-certificates curl gnupg lsb-release jq
...
apt update && apt install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin kubectl helm
